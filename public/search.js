        ...
        axios
          ...
          .then((response) => {
            ...
            for (let i = 0; i < results.length; i++) {
              ...
              let suggestionItemEmail = document.createElement('p');
              suggestionItemEmail.textContent = result.email;
              ...
              suggestionItemEmail.classList.add(
                'text-sm',
                );
              ...
              suggestionItem.appendChild(suggestionItemEmail);
...
//Searching
export function setupSearchParents(searchInput, userIdInput) {
    if (!searchInput || !userIdInput) {
      return; // exit early if inputs don't exist
    }
            ...
            for (let i = 0; i < results.length; i++) {
              ...
              suggestionItemEmail.classList.add(
                ...
                suggestionItem.addEventListener('click', function () {
                    searchInput.value = result.name;
                    userIdInput.value = result.id;
                    Livewire.emit('selectedParent', 'parent_id', result.id);
