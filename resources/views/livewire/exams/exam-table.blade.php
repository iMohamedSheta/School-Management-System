                ...
                <tr class="hover:bg-gray-50">
                    <td>
                        <div class="flex items-center mr-4">
                            ...
                            <input id="classroom-{{ $exam->id }}-checkbox" type="checkbox" value="{{ $exam->id }}" style="color:#cb0c9f;" class="w-4 h-4  bg-gray-100 border-gray-300 rounded focus:ring-0   dark:ring-offset-gray-800 focus:ring-1 dark:bg-gray-700 dark:border-gray-600">
                        </div>
                    </td>
                    <td class="px-6 py-3 whitespace-nowrap text-center overflow-auto simplebar " >
                        {{ getIteration($exams,$loop) }}
                    </td>
                    <td class="px-6 py-3 whitespace-nowrap text-center overflow-auto simplebar" >
                    ...
                    <td class="px-6 py-3 whitespace-nowrap text-center">
                        <livewire:exams.exam-information :exam="$exam" :wire:key="'info'.$exam->id"/>
                        <livewire:exams.exam-edit :exam='$exam' :wire:key="'edit'.$exam->id"/>
                        <livewire:exams.exam-delete :exam="$exam" :wire:key="'delete'.$exam->id" />
     ...
     <div id="modal-deleteAll" wire:ignore tabindex="-1" aria-hidden="true" class="fixed top-0 left-0 right-0 z-50 hidden max-md:w-screen max-md:h-screen p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-modal md:h-full">
        <div class="relative w-full h-full max-w-md md:h-auto">
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <form action="{{route('exams.selected.destroy')}}" method="post">
                    @method("delete")
                    @csrf
                <div class="p-6 text-center">
                    <svg aria-hidden="true" class="mx-auto mb-4 text-gray-400 w-14 h-14 dark:text-gray-200" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                    <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">{{trans('main.delete-exams-title')}}</h3>
