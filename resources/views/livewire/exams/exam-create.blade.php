...
<div class="w-full sm:max-w-md lg:max-w-2xl mt-0 px-6 py-4 bg-white shadow-md overflow-hidden sm:rounded-lg mx-auto">
    <div class="pb-10 pt-2 w-full">
        <div class="text-xl pb-4 pt-2">{{ trans('main.add-exam') }}</div>

        <form wire:submit.prevent="createExam" method="post">
            @csrf
            ...
            <button type="submit" wire:submit='createExam' wire:loading.attr="disabled"  class="btn btn-primary bg-gray-800 text-white w-full mt-4" >
                {{ __('main.add-the-exam') }}
            </button>
        </form>
