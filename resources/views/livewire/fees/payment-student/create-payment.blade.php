...
<div>
    ...
    <div class="w-full sm:max-w-md lg:max-w-2xl mt-0 px-6 py-4 bg-white shadow-md overflow-hidden sm:rounded-lg mx-auto">
        <div class="pb-10 pt-2 w-full">
            <div class="text-xl pb-4 pt-2 mb-4">{{ trans('main.add-payment-title',['name'=>$student->name]) }}</div>

            <form wire:submit.prevent="createPayment" method="post">
                @csrf
                ...
                <button type="submit" wire:submit.prevent='createPayment' wire:loading.attr='disabled' class="btn btn-primary bg-gray-800 text-white w-full mt-4" >
                    {{ __('main.add-payment') }}
                </button>
            </form>
