                ...
                <tr class="hover:bg-gray-50">
                    <td>
                        <div class="flex items-center mr-4">
                            ...
                            <input id="classroom-{{ $receipt->id }}-checkbox" type="checkbox" value="{{ $receipt->id }}" style="color:#cb0c9f;" class="w-4 h-4  bg-gray-100 border-gray-300 rounded focus:ring-0   dark:ring-offset-gray-800 focus:ring-1 dark:bg-gray-700 dark:border-gray-600">
                        </div>
                    </td>
                    <td class="px-6 py-3 whitespace-nowrap text-center overflow-auto simplebar " >
                        {{ getIteration($receipts,$loop) }}
                    </td>
                    <th scope="row" class="flex items-center text-cemter justify-center px-6 py-2 text-gray-900 whitespace-nowrap dark:text-white">
                        <img class="object-cover mx-1 w-10 h-10 rounded-full" src="{{ $receipt->student->user->profile_photo_url }}" alt="{{ $receipt->student->user->name }}" />
                        <div class="pl-3">
                    ...
                    <td class="px-6 pb-3 pt-4 whitespace-nowrap text-center overflow-auto simplebar" >
                        <span class="bg-red-100 text-red-800 text-sm font-mono font-bold mr-2 px-2.5 py-0.5 rounded dark:bg-red-900 dark:text-red-300">
                            {{ $receipt->formatted_amount ??  trans('main.undefined') }}
                        </span>
                    </td>
                    ...
                    <td class="px-6 py-3 whitespace-nowrap text-center overflow-auto simplebar" >
                        ...
                        {{ $receipt->date ? MyCarbon::parse($receipt->date)->format('d-m-Y') : trans('main.undefined') }}
                    </span>
                    </td>
                    <td class="px-6 py-3 whitespace-nowrap text-center">
                        <livewire:fees.receipts.receipt-information :receipt="$receipt" :wire:key="'info'.$receipt->id">
                        <livewire:fees.receipts.receipt-edit :receipt='$receipt' :wire:key="'edit'.$receipt->id">
                        <livewire:fees.receipts.receipt-delete :receipt="$receipt" :wire:key="'delete'.$receipt->id">
     ...
     <div id="modal-deleteAll" wire:ignore tabindex="-1" aria-hidden="true" class="fixed top-0 left-0 right-0 z-50 hidden max-md:w-screen max-md:h-screen p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-modal md:h-full">
        <div class="relative w-full h-full max-w-md md:h-auto">
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <form action="{{route('receipts.selected.destroy')}}" method="post">
                    @method("delete")
                    @csrf
