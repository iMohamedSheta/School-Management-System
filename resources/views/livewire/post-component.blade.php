...
<div class="p-6 border-y-8 rounded-2xl sm:rounded-lg ">
    ...
    <div class="bg-white p-4 w-[80%] mx-auto rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">



        <div class="text-xl my-2">{{ trans('main.get-discussions-based-on-grade') }}</div>
        <div class="grid md:grid-cols-2 md:gap-6">
            <div class="relative z-0 w-full  group ">
                <select id="grade_id" wire:model.defer="grade_id" name="grade_id" autocomplete="off" class="block py-2.5  px-0 w-full text-sm text-gray-500 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-gray-400 dark:border-gray-700 focus:outline-none focus:ring-0 focus:border-blue-600 peer" wire:model="selectedGrade" onchange="Livewire.emit('updatedGradeId', this.value)">
...
<div class=' md:w-[90%] mx-auto my-8 hs:w-full  '>
    <div class="max-w-2xl mx-auto bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
        <div class="border-b-4">
            <div class="flex justify-between items-center text-white  px-3 pt-4 pb-2 max-w-2xl mx-auto   bg-gray-800">
                ...
                <div class="flex flex-row-reverse justify-center items-center ">
                    <div>
                        <button id="dropdownMenuIconHorizontalButton-{{$post->id}}" data-dropdown-toggle="dropdownDotsHorizontal-{{$post->id}}" class="text-white hover:text-gray-200" type="button">
                        ...
                        <div>
                            ...
                            <button  class="hover:text-red-500" type="button" data-modal-target="delete-post-{{$post->id}}" id="delete-post-btn-{{$post->id}}"  data-modal-toggle="delete-post-{{$post->id}}">
                                <i class="fa-regular fa-trash-can  fa-xl mx-4"></i>
                            </button>
                            <div>
    ...
    <div class="">
        ...
        <ul class="py-2 bg-gray-100 px-5">
            ...
            <li class="flex ">
                ...
                <div class="w-11/12 relative border-2 shadow-lg my-3 rounded-2xl px-3 pb-3 pt-1 mx-2 bg-white">
                    <div class="">
                        ...
                        <div class="absolute top-0 {{ in_array(app()->getLocale(), ['ar', 'he', 'fa', 'ur']) ? 'left-0' : 'right-0'}} "  x-data="{ dropdownOpen: false }">
                            <button @click="dropdownOpen = !dropdownOpen" class="inline-flex pt-4 px-4 items-center  text-sm font-medium text-center text-gray-900 bg-white rounded-lg focus:outline-none dark:text-white focus:ring-gray-50 dark:bg-gray-800 dark:hover:bg-gray-700 dark:focus:ring-gray-600" type="button">
                            ...
                            <div x-show='dropdownOpen' @click.away='dropdownOpen = false'  class="absolute right-0 mt-2 w-full rounded-md shadow-lg bg-white z-10">
                                @if( (Auth::user()->id ==$comment->user->id))
                                <ul class=" text-sm text-gray-700 dark:text-gray-200 " >
                                    <li>
                                        <div x-data="{ showModal: false}">
                                            <button x-on:click="showModal = true" class="block text-center w-full px-4 py-3 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white"><i class="fa-solid fa-pencil "></i></button>
                                            <!-- edit comment modal -->
                                            <div x-show="showModal"  aria-hidden="true" class="fixed top-0 left-0 right-0 bg-gray-600 bg-opacity-60 z-50 max-md:w-screen max-md:h-screen md:py-[15%] md:px-[32%] overflow-x-hidden overflow-y-auto md:inset-0 h-modal md:h-full">
                                                <div class="relative w-full h-full max-w-2xl md:h-auto"  x-on:click.away="showModal = false" >
                                                        <!-- Modal content -->
                                                        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700 ">
                                                            <button type="button" x-on:click="showModal = false" class="absolute top-3 {{ in_array(app()->getLocale(), ['ar', 'he', 'fa', 'ur']) ? 'left-2.5' : 'right-2.5' }}  text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-800 dark:hover:text-white">
                                                                <svg aria-hidden="true" class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"></path></svg>
                                                                <span class="sr-only">Close modal</span>
                                                            </button>
                                                            <div class="px-6 py-6 lg:px-8">
                                                                ...
                                                                <form class="space-y-6" wire:submit.prevent='updateComment({{$comment->id}})' method="post">
                                                                    @csrf
                                                                    <div>
                                                                        <textarea id="message" wire:model="editedComment" rows="4" class="block p-2.5  w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" ></textarea>
                                                                    </div>
                                ...
                                @endif
                                    <div class="">
                                        <button type="button" x-on:click="dropdownOpen = false" wire:click='deleteComment({{$comment->id}})' class="block w-full px-4 py-3 text-sm rounded-b-lg text-white bg-red-600 hover:bg-red-500 dark:hover:bg-gray-600 dark:text-gray-200 dark:hover:text-white"><i class="fa-solid fa-trash "></i></button>
                                    </div>
                            </div>
                        </div>
                        @endif
...
@endforelse
@if($posts->lastPage() > 1 && $posts->lastPage() <= 10)
    <div class="bg-white sm:rounded-lg shadow-xl p-6">
    {{$posts->render()}}
    </div>
    @endif
