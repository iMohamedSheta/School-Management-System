...
<table class="min-w-full max-w-7xl divide-y divide-gray-200 text-sm" id="multiselect-table">
    ...
    <tbody class="bg-white divide-y divide-gray-200">
        ...
        @forelse ($teacherclassrooms as $teacherclassroom)
            <tr class="hover:bg-gray-50">

                <td>
                    <div class="flex items-center mr-4">
                        <label for="red-checkbox" class="ml-2 text-sm font-medium text-gray-900 dark:text-gray-300"></label>
                        <input id="teacherclassroom-{{ $teacherclassroom->id }}" type="checkbox" value={{$teacherclassroom->id}} style="color:#cb0c9f;" class="w-4 h-4  bg-gray-100 border-gray-300 rounded focus:ring-0   dark:ring-offset-gray-800 focus:ring-1 dark:bg-gray-700 dark:border-gray-600">
                    </div>
                </td>

                <td class="px-6 py-3 whitespace-nowrap text-center overflow-auto simplebar " >
                    {{ getiteration($teacherclassrooms,$loop) }}
                </td>
                <th scope="row" class="flex items-center text-cemter justify-center px-6 py-2 text-gray-900 whitespace-nowrap dark:text-white">
                    <img class="object-cover mx-1 w-10 h-10 rounded-full" src="{{ $teacherclassroom->teacher->user->profile_photo_url }}" alt="{{ $teacherclassroom->teacher->user->name }}" />
                    <div class="pl-3">
                        <div class="text-base font-semibold">{{ $teacherclassroom->teacher->teacher_name ?? '-' }}</div>
                        <div class="font-normal text-gray-500">
                            {{$teacherclassroom->teacher->user->email}}
                        </div>
                    </div>
                </th>
                ...
                <td class="px-6 py-3 whitespace-nowrap text-center overflow-auto simplebar " >
                    <span class="bg-purple-100 text-purple-800 text-sm font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-purple-900 dark:text-purple-300">
                        {{ $teacherclassroom->classroom->grade->name ?? '-' }} 
                    </span>
                </td>
                <td class="px-6 py-3 whitespace-nowrap text-center overflow-auto simplebar" >
                    <span class="bg-blue-100 text-blue-800 text-sm font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-blue-900 dark:text-blue-300">
                        {{ $teacherclassroom->classroom->name ?? '-' }}
                    </span>
                </td>
                <td class=" py-3 whitespace-nowrap text-center  absolute">

                            <div x-data="{ showModal: false }">
                                <button class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 hover:text-gray-900 w-full" x-on:click="showModal = true" ><i class="mx-2 fa-solid fa-trash text-red-600 hover:text-red-500 fa-lg"></i>{{ trans('main.delete-teacher-classroom-assigned-dropdown') }}
                                </button>
                                    <!-- Delete modal -->
                                    ...
                                    <div class="relative w-full h-full max-w-xl md:h-auto"  x-on:click.away="showModal = false">
                                        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700 ">
                                            ...
                                            <form action="{{route('teacher.remove.assign.classroom')}}" method="post" class="inline">
                                                @method("delete")
                                                @csrf
                                                <div class="p-6 text-center">
                                                    <svg aria-hidden="true" class="mx-auto mb-4 text-gray-400 w-14 h-14 dark:text-gray-200" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                                                    <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">{{trans('main.delete-teacher-assign-classroom-title',['name'=>$teacherclassroom->teacher->teacher_name,'classroom'=>$teacherclassroom->classroom->name])}}</h3>
                                                    <input class="text" type="hidden"  name="teacherClassroomIdForDelete" value="{{$teacherclassroom->id}}">
 ...
 <div id="modal-deleteAll" tabindex="-1" aria-hidden="true" class="fixed top-0 left-0 right-0 z-50 hidden max-md:w-screen max-md:h-screen p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-modal md:h-full">
    <div class="relative w-full h-full max-w-xl md:h-auto">
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <form action="{{route('teacher.remove.selected.assign.classroom')}}" method="post">
                @method("delete")
                @csrf
            <div class="p-6 text-center">
                <svg aria-hidden="true" class="mx-auto mb-4 text-gray-400 w-14 h-14 dark:text-gray-200" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">{{trans('main.remove-teachers-assigned-classrooms-title')}}</h3>
