...
<div class="w-full sm:max-w-md lg:max-w-3xl mt-0 px-6 py-4 bg-white shadow-md overflow-hidden sm:rounded-lg mx-auto">
    <div class="pb-4 pt-2 w-full">
        <div class="text-xl font-semibold px-2 pb-3 pt-2">{{ trans('main.export-attendance-on-date') }}</div>
        <div class="text-md px-6 pt-4 text-gray-500 border-t-2 mt-2 mb-6">
            {{ trans('main.assign_classroom_info_friendly') }}
        </div>

        <div class="mx-4">
            <form wire:submit.prevent="exportAttendanceForDate()" method="post">
                @csrf
            <div class="grid md:grid-cols-2 md:gap-6">
                <div class="relative z-0 w-full mb-6 group">
                    ...
                    <label for="name" class="peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:right-0 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">{{ trans('main.report-name') }}</label>
                    @error('name')
                ...
                <div class="relative max-w-sm">
                    ...
                    <input   id="date"  type="text" autocomplete="off"  class="block py-2.5  px-0 w-full text-sm text-gray-500 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-gray-400 dark:border-gray-700 focus:outline-none focus:ring-0 focus:border-blue-600 peer" wire:model.defer="date"  value  placeholder="{{trans('main.date')}}">
                    @error('date')
                ...
                <button type="submit" wire:submit='exportAttendanceForDate' wire:loading.attr='disabled'  class="btn btn-primary bg-gray-800 text-white w-full mt-4" >
                {{ __('main.export-report') }}
                </button>
            </form>
        </div>
        </div>
